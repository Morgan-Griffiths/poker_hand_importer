...
def return_max_potlimit_betsize(
    ...
    if previous_aggro_action is None:
        ...
        min_betsize = 0
    elif previous_aggro_action.action_type == ActionType.BET:  # Bet
    ...
    elif previous_aggro_action.action_type == ActionType.RAISE:  # Raise
        max_betsize = min(
...
def convert_cards(cards: list[tuple[str, str]]):
    hole_cards = []
    for card in cards:
...
def process_players(
    ...
    for player in players:
        if player.hole_cards:
            player.hole_cards = convert_cards(player.hole_cards)
        else:
            player.hole_cards = [0, 0, 0, 0] * 2
        player_obj = Player(
            stack=player.stack,
...
def calculate_last_agro_action(
    action: Action, last_agro_action: Action, position_to_int: dict[str, int]
):
    # last agro action
...
class MLConversion:
    ...
    def convert_hands(self, hands: list[PokerHand]):
    ...
    def record_hand(
        ...
        for i in range(1, 6):
            next_state[state_mapping[f"vil{i}_active"]] = padded_villains[
                i - 1
            ].is_active
            next_state[state_mapping[f"vil{i}_stack"]] = padded_villains[i - 1].stack
            next_state[state_mapping[f"vil{i}_position"]] = POSITION_TO_SEAT[
                padded_villains[i - 1].position
            ]
        ...
        next_state[state_mapping["next_player"]] = (
            POSITION_TO_SEAT[next_players[1]] if len(next_players) > 1 else 0
        )

    def convert_hand(self, hand: PokerHand):
        """
        ...
        if not hero or any(
            [True if p.is_active and p.stack == 0 else False for p in players]
        ):
            return [], [], []
        ...
        for i, (action, round_stats, round_seats) in enumerate(
            ...
            if isinstance(action, Street):
                ...
                hand_board = (
                    hero.hole_cards + board + [0 for _ in range(10 - len(board))]
                )
                next_state[state_mapping["pot"]] = round_stats.pot
                # postflop ordering minus active players
                next_players = deque(
                    [p for p in postflop_positions if p in active_positions]
                )
            ...
            padded_villains = [p for p in players if p.position != hero.position] + [
                Player(
                    stack=0,
                    hole_cards=[0] * 8,
                    position=None,
            ...
            assert POSITION_TO_SEAT[hero.position] not in [
                POSITION_TO_SEAT[p.position] for p in padded_villains
            ], "hero position overlaps with villain positions"
        ...
        # lambda discounted rewards
        target_rewards = [
            (result / bb) * 0.95**i for i in range(num_hero_decisions - 1, -1, -1)
        ]
