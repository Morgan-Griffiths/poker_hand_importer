    ...
    try:
        for e in range(training_params["epochs"]):
            ...
            while not done:
                ...
                player_state = pokerrl.player_view(global_states,current_player,config)
                outputs = model(torch.tensor(player_state).to(device).unsqueeze(0)).squeeze(0).detach().cpu()
