...
class PreProcess(nn.Module):
    ...
    def forward(self, state):
        ...
        vil3_active_emb = self.emb_active(state[:, :, state_mapping["vil3_active"]].long())
        vil4_active_emb = self.emb_active(state[:, :, state_mapping["vil4_active"]].long())
        vil5_active_emb = self.emb_active(state[:, :, state_mapping["vil5_active"]].long())
        vil1_position_emb = self.emb_position(state[:, :, state_mapping["vil1_position"]].long())
        vil2_position_emb = self.emb_position(state[:, :, state_mapping["vil2_position"]].long())
        vil3_position_emb = self.emb_position(state[:, :, state_mapping["vil3_position"]].long())
        vil4_position_emb = self.emb_position(state[:, :, state_mapping["vil4_position"]].long())
        vil5_position_emb = self.emb_position(state[:, :, state_mapping["vil5_position"]].long())
        last_agro_action_emb = self.emb_action(state[:, :, state_mapping["last_agro_action"]].long())
        last_agro_position_emb = self.emb_position(state[:, :, state_mapping["last_agro_position"]].long())
        last_agro_is_blind_emb = self.emb_is_blind(state[:, :, state_mapping["last_agro_is_blind"]].long())
        previous_position_emb = self.emb_position(state[:, :, state_mapping["previous_position"]].long())
        previous_action_emb = self.emb_action(state[:, :, state_mapping["previous_action"]].long())
        previous_bet_is_blind_emb = self.emb_is_blind(state[:, :, state_mapping["previous_bet_is_blind"]].long())
        next_player_emb = self.emb_position(state[:, :, state_mapping["next_player"]].long())

        # Continuous features
        last_agro_amount = self.process_last_agro_amount(state[:, :, state_mapping["last_agro_amount"]].unsqueeze(-1).float().float())
        pot = self.process_pot(state[:, :, state_mapping["pot"]].unsqueeze(-1).float())
        amount_to_call = self.process_amount_to_call(state[:, :, state_mapping["amount_to_call"]].unsqueeze(-1).float())
        pot_odds = self.process_pot_odds(state[:, :, state_mapping["pot_odds"]].unsqueeze(-1).float())
        previous_amount = self.process_previous_amount(state[:, :, state_mapping["previous_amount"]].unsqueeze(-1).float())
        hero_stack = self.process_stack(state[:, :, state_mapping["hero_stack"]].unsqueeze(-1).float())
        vil1_stack = self.process_stack(state[:, :, state_mapping["vil1_stack"]].unsqueeze(-1).float())
        vil2_stack = self.process_stack(state[:, :, state_mapping["vil2_stack"]].unsqueeze(-1).float())
        vil3_stack = self.process_stack(state[:, :, state_mapping["vil3_stack"]].unsqueeze(-1).float())
        vil4_stack = self.process_stack(state[:, :, state_mapping["vil4_stack"]].unsqueeze(-1).float())
        vil5_stack = self.process_stack(state[:, :, state_mapping["vil5_stack"]].unsqueeze(-1).float())
